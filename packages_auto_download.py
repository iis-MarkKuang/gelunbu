...
def download_dnf_packages(target_folder):
    if not os.path.exists(target_folder):
        os.makedirs(target_folder)
    dept_list = ["gcc-c++", "python3-devel", "libstdc++-devel"]
    for dept in dept_list:
        command = ["dnf", "download", "--arch=" + get_architecture(), dept, "--downloaddir=" + target_folder]
        print(f"Running command: {' '.join(command)}")
        subprocess.run(command, check=True)
...
if __name__ == "__main__":
    ...
    pip_target_folder = './pip_packages'
    download_dnf_packages(dep_target_folder)
    download_pip_packages('./requirements_station.txt', pip_target_folder, "")
